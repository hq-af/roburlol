...
function Tristana.OnPostAttack(target)
    if target.IsHero and target.IsEnemy and API.Orbwalker.GetMode() == "Combo" and Tristana.Q:IsReady() then
        Tristana.Q:Cast()
    end
end
...
function Tristana.OnDraw()
    ...
    if Tristana.W:IsReady() then
        API.Renderer.DrawCircle3D(API.Player.Position, Tristana.W.Range, 30)
    end
...
function Tristana.OnTick()
    ...
    if API.Orbwalker.GetMode() == "Combo" and Tristana.E:IsReady() and (API.Player.Pathing.IsDashing or API.Orbwalker.CanMove()) then
